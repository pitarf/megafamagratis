...
if ! docker compose --env-file .env.production -f docker-compose.prod.yml ps | grep -q "megafama_postgres"; then
  echo "❌ ERRO: O container de banco de dados 'megafama_postgres' não está em execução."
  exit 1
fi

echo "📥 Extraindo dump do banco de dados..."
if ! docker compose --env-file .env.production -f docker-compose.prod.yml exec -T postgres pg_dump -U "$POSTGRES_USER" -d "$POSTGRES_DB" -Fc > "$BACKUP_FILE"; then
...
if ! docker compose --env-file .env.production -f docker-compose.prod.yml exec -T postgres pg_restore --list < "$BACKUP_FILE" > /dev/null; then
